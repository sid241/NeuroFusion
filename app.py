...
app = Flask(__name__)
app.secret_key = 'super_secret_key_neurofusion_v2'
...
# VGG19 CNN For Fusion (only if torch is available)
if HAVE_TORCH:
...
class Fusion:
    def __init__(self, input):
        ...
        If PyTorch isn't available we fall back to a simple average-based
        fusion so the web app can run with reduced functionality.
        """
    ...
    def fuse(self):
        """High-level fusion entry point. Uses the deep model when
        available and a simple average fusion fallback otherwise.
        """
        if not self._use_torch:
            imgs = [img.astype(np.float32) for img in self.input_images]
            base_shape = imgs[0].shape
            resized = []
            for im in imgs:
                if im.shape != base_shape:
                    imr = cv2.resize(im, (base_shape[1], base_shape[0]))
                else:
                    imr = im
                resized.append(imr)
            out = np.mean(np.stack(resized, axis=0), axis=0)
            out = np.clip(out, 0, 255).astype(np.uint8)
            return out

        # Torch-based fusion follows original implementation
...
def login():
    ...
    if request.method == 'POST':
        if request.form.get('username') == 'doctor' and request.form.get('password') == 'doctor123':
...
def segmentation():
    ...
    # Detailed AI Engine Analysis: Classify the entire segmented brain composition (adding to 100%)
    seg_img = cv2.imread('static/segmented.jpg')
    seg_rgb = cv2.cvtColor(seg_img, cv2.COLOR_BGR2RGB)
    seg_hsv = cv2.cvtColor(seg_rgb, cv2.COLOR_RGB2HSV)
    
    # Filter out black background (Value > 15)
    brain_mask = seg_hsv[:,:,2] > 15
    h_channel = seg_hsv[:,:,0]
    
    # Classify hues strictly (OpenCV Hue is 0-179)
    # Red/Pink regions
    red_mask = brain_mask & ((h_channel < 15) | (h_channel > 150))
    # Green regions
    green_mask = brain_mask & (h_channel >= 35) & (h_channel <= 85)
    # Remaining regions (Blue/Cyan/Yellow/Grey)
    other_mask = brain_mask & ~(red_mask | green_mask)
    
    red_px = np.sum(red_mask)
    green_px = np.sum(green_mask)
    other_px = np.sum(other_mask)
    
    total_px = red_px + green_px + other_px
    if total_px == 0: total_px = 1 # Fallback
    
    red_percent = (red_px / total_px) * 100
    green_percent = (green_px / total_px) * 100
    other_percent = (other_px / total_px) * 100
    
    # Advanced logic for conclusion
    if green_percent > 15.0 or red_percent > 35.0:
        conclusion = "Significant structural deviation detected. AI segmentation identified widespread abnormal density mapping inside the neurological boundaries."
        safety_level = "Critical"
        safety_color = "danger"
    elif green_percent > 5.0 or red_percent > 15.0:
        conclusion = "Moderate structural anomalies detected. The scan indicates growing irregular structural mappings."
        safety_level = "A bit serious"
        safety_color = "warning"
    else:
        conclusion = "No critical abnormal structural densities detected. The brain matter segmentation remains optimally balanced."
        safety_level = "Safe"
    ...
    return render_template("segmentation.html", report=report)
